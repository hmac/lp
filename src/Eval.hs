...
nfc' ctx ex = head (reduceList' ex)
  where reduceList' expr = go [expr]
        go (e : es) = let e' = reduce' e
                       in if e' == e
                          then e : es
                          else go (e' : e : es)
        go x = x
...
breduce (Fix (Lam _ (Fix e))) (Fix b) =
  ...
  -- Remove lambda
   in Fix e''
  where sub b i (Var j) | i == j = b
                        | otherwise = Var j
        sub b i (Lam v (Fix e)) = Lam v (Fix (sub b (i + 1) e))
        sub b i (Pi x (Fix t) (Fix e)) = Pi x (Fix (sub b i t))
                                              (Fix (sub b (i + 1) e))
        sub b i Type = Type
        sub b i (Ann (Fix e) (Fix t)) = Ann (Fix (sub b i e))
                                            (Fix (sub b i t))
        sub b i (App (Fix e1) (Fix e2)) = App (Fix (sub b i e1))
                                              (Fix (sub b i e2))

        decFree i (Var j) | j > i = Var (j - 1)
                          | otherwise = Var j
        decFree i (Lam v (Fix e)) = Lam v (Fix (decFree (i + 1) e))
        decFree i (Pi x (Fix t) (Fix e)) = Pi x (Fix (decFree i t))
                                                (Fix (decFree (i + 1) e))
        decFree i Type = Type
        decFree i (Ann (Fix e) (Fix t)) = Ann (Fix (decFree i e))
                                              (Fix (decFree i t))
        decFree i (App (Fix a) (Fix b)) = App (Fix (decFree i a))
                                              (Fix (decFree i b))
